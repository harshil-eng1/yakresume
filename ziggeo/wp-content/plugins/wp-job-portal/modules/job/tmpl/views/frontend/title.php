...
switch ($layout) {
    ...
    if(in_array('multicompany', wpjobportal::$_active_addons)){
        ...
        $url = $url = wpjobportal::wpjobportal_makeUrl(array('wpjobportalme'=>'company', 'wpjobportallt'=>'viewcompany', 'wpjobportalid'=>$job->companyid));
    } ?>
                    ...
                    <div class="wjportal-jobs-info">
                        <div class="wjportal-jobs-salary">
                            ...
                            <?php if($job->salarytype==3 || $job->salarytype==2) { ?>
                                <span class="wjportal-salary-type"> <?php echo ' / ' .esc_html(__(esc_html($job->srangetypetitle), 'wp-job-portal')) ?></span>
                            <?php }?>
                        </div>
                    </div>
                    <div class="wjportal-jobs-info">
                        <?php 
                            ...
                            echo esc_html(human_time_diff(strtotime($job->created)).' '.esc_html(__("Ago",'wp-job-portal')));
                    ...
                    echo '</div>
                        <div class="wjportal-job-data-wrp">
                            <div class="wjportal-job-desc">
                                <div class="wjportal-job-sec-title">'. esc_html(__("Description",'wp-job-portal')) .'</div>
        ...
        case 'job_seeker':
            echo ' <div class="wjportal-jobinfo-wrp">';
                   ...
                   if(isset($job) && !empty($job->salarytype)){
                     echo '<div class="wjportal-jobinfo">
                            <span class="wjportal-jobinfo-data">
                                <img src="' . WPJOBPORTAL_PLUGIN_URL . 'includes/images/salary.png" alt="'.esc_html(__("salary",'wp-job-portal')).'" title="'.esc_html(__("salary",'wp-job-portal')).'" />
                                ...
                                <span class="wjportal-salary-type"> <?php  echo ' / ' .wpjobportal::wpjobportal_getVariableValue($job->srangetypetitle); ?></span>
                            <?php }
                     echo '</div>';
                    }
                    if(isset($job) && !empty($job->cat_title)){
                        echo '<div class="wjportal-jobinfo">
                                <span class="wjportal-jobinfo-data">
                                    <img src="' . WPJOBPORTAL_PLUGIN_URL . 'includes/images/category.png" alt="'.esc_html(__("category",'wp-job-portal')).'" title="'.esc_html(__("category",'wp-job-portal')).'" />
                    ...
                    if(isset($job) && $job->created){
                        echo ' <div class="wjportal-jobinfo">
                                    <span class="wjportal-jobinfo-data">
                                        <img src="' . WPJOBPORTAL_PLUGIN_URL . 'includes/images/calander.png" alt="'.esc_html(__("created",'wp-job-portal')).'" title="'.esc_html(__("created",'wp-job-portal')).'" />
                   ...
                   if(isset($job) && $job->stoppublishing){
                        echo '<div class="wjportal-jobinfo">
                                    <span class="wjportal-jobinfo-data wjportal-job-close-date">
                                        <img src="' . WPJOBPORTAL_PLUGIN_URL . 'includes/images/end-date.png" alt="'.esc_html(__("end date",'wp-job-portal')).'" title="'.esc_html(__("end date",'wp-job-portal')).'" />'.esc_html(__('Closes','wp-job-portal')).':
                   ...
                   if(isset($job) && !empty($job->multicity)){
                        echo '<div class="wjportal-jobinfo">
                                <span class="wjportal-jobinfo-data">
                                    <img src="' . WPJOBPORTAL_PLUGIN_URL . 'includes/images/locationn.png" alt="'.esc_html(__("location",'wp-job-portal')).'" title="'.esc_html(__("location",'wp-job-portal')).'"/>
    ...
    case 'apply1':
        ...
        if(!WPJOBPORTALincluder::getObjectClass('user')->isguest()){
            ...
            if(wpjobportal::$_config->getConfigValue('showapplybutton') == 1){
                if(in_array('credits', wpjobportal::$_active_addons)){
                    ...
                    if(isset($applied) && !empty($applied)){
                        if($applied->no == 1){
                            echo'<span class="wjportal-job-company-apply-status" onclick="getApplyNowByJobid('.esc_js($job->id).')">'. esc_html(__("You Already Applied to this job ",'wp-job-portal')) .' </span>';
                        }elseif ($applied->no == 0) {
                            echo '<a class="wjportal-job-company-btn" onclick="getApplyNowByJobid('.esc_js($job->id).')">' . esc_html(__('Apply On This Job', 'wp-job-portal')).'</a>';
                        }
                    }else{
                        echo '<a class="wjportal-job-company-btn" onclick="getApplyNowByJobid('.esc_js($job->id).')">' . esc_html(__('Apply On This Job', 'wp-job-portal')).'</a>';
                    }
            ...
            if(in_array('credits', wpjobportal::$_active_addons) && $visitorcanapply != 1){
                ...
                echo '<a href="'.$finalurl.'" class="wjportal-job-company-btn" title="' . esc_html(__('Login to Apply On This Job', 'wp-job-portal')) . '">' . esc_html(__('Login to Apply On This Job', 'wp-job-portal')) . '</a>';
            } else {
                ...
                if ($visitor_show_login_message == 1) {
                    ...
                    echo '<a class="wjportal-job-company-btn" href="' . wpjobportal::wpjobportal_makeUrl(array('wpjobportalme'=>'jobapply', 'action'=>'wpjobportaltask', 'task'=>'jobapplyasvisitor', 'wpjobportalid-jobid'=>$job->id, 'wpjobportalpageid'=>wpjobportal::wpjobportal_getPageid())) . '">' . esc_html(__('Apply On This Job', 'wp-job-portal')).'</a>';
                }
            }
        }
        ...
        if(!WPJOBPORTALincluder::getObjectClass('user')->isguest()){
            ...
            if(in_array('credits', wpjobportal::$_active_addons)){
                ...
                if(wpjobportal::$_config->getConfigValue('showapplybutton') == 1){
                    if(isset($applied) && !empty($applied)){
                        if($applied->no != 1){
                            echo '<a class="wjportal-job-act-btn" onclick="getApplyNowByJobid('.esc_js($job->id).','.esc_js($package).')">' . esc_html(__('Apply On This Job', 'wp-job-portal')).'</a>';
                        }
                    }
                }
            }
            ...
            if(in_array('credits', wpjobportal::$_active_addons) && $visitorcanapply != 1){
                $finalurl = wpjobportal::wpjobportal_makeUrl(array('wpjobportalme'=>'wpjobportal', 'wpjobportallt'=>'login'));
                echo '<a href="'.$finalurl.'" class="wjportal-job-act-btn" title="' . esc_html(__('Login to Apply On This Job', 'wp-job-portal')) . '">' . esc_html(__('Login to Apply On This Job', 'wp-job-portal')) . '</a>';
            } else {
                $visitor_show_login_message = wpjobportal::$_config->getConfigurationByConfigName('visitor_show_login_message');
                if ($visitor_show_login_message == 1) {
                    echo '<a class="wjportal-job-act-btn" onclick="getApplyNowByJobid('.esc_js($job->id).')">' . esc_html(__('Apply On This Job', 'wp-job-portal')).'</a>';
                } else {
                    echo '<a class="wjportal-job-act-btn" href="' . wpjobportal::wpjobportal_makeUrl(array('wpjobportalme'=>'jobapply', 'action'=>'wpjobportaltask', 'task'=>'jobapplyasvisitor', 'wpjobportalid-jobid'=>$job->id, 'wpjobportalpageid'=>wpjobportal::wpjobportal_getPageid())) . '">' . esc_html(__('Apply On This Job', 'wp-job-portal')).'</a>';
                }
            }
        }
         # Social Share
