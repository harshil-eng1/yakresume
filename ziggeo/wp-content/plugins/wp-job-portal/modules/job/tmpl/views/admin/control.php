 	...
 	case 'control':
        ...
        <div id="for_ajax_only_<?php echo esc_attr($job->id); ?>">
            <div id="item-actions" class="wpjobportal-jobs-action-wrp">
                <a class="wpjobportal-jobs-act-btn" href="<?php echo esc_url(admin_url('admin.php?page=wpjobportal_job&wpjobportallt=formjob&wpjobportalid='.$job->id)); ?>" title="<?php echo esc_html(__('edit', 'wp-job-portal')); ?>">
                    <?php echo esc_html(__('Edit', 'wp-job-portal')); ?>
                </a>
                <a class="wpjobportal-jobs-act-btn" href="<?php echo esc_url(wp_nonce_url(admin_url('admin.php?page=wpjobportal_job&action=wpjobportaltask&task=remove&callfrom=1&wpjobportal-cb[]='.$job->id),'delete-job')); ?>" onclick='return confirm("<?php echo esc_html(__('Are you sure to delete','wp-job-portal')).' ?'; ?>");' title="<?php echo esc_html(__('delete', 'wp-job-portal')); ?>">
                    <?php echo esc_html(__('Delete', 'wp-job-portal')); ?>
                </a>
                <a class="wpjobportal-jobs-act-btn" href="<?php echo esc_attr(wp_nonce_url(admin_url('admin.php?page=wpjobportal_job&action=wpjobportaltask&callfrom=1&task=jobenforcedelete&jobid='.$job->id),'delete-job')); ?>" onclick='return confirmdelete("<?php echo esc_html(__('This will delete every thing about this record','wp-job-portal')).'. '.esc_html(__('Are you sure to delete','wp-job-portal')).' ?'; ?>");' title="<?php echo esc_html(__('force delete', 'wp-job-portal')) ?>">
                ...
                <a class="wpjobportal-jobs-act-btn wpjobportal-jobs-apply-res" href="<?php echo esc_attr(admin_url('admin.php?page=wpjobportal_jobapply&wpjobportallt=jobappliedresume&jobid='.$job->id)); ?>" title="<?php echo esc_html(__('Applied Resume', 'wp-job-portal')); ?>">
        ...
        $html.='<div class="wpjobportal-jobs-action-wrp">';
                    ...
                    if ($total == 1) {
                        if (isset($arr['self'])) {
                            ...
                            $html.='<a class="wpjobportal-jobs-act-btn" href="admin.php?page=wpjobportal_job&task=approveQueueJob&id='.$job->id.'&action=wpjobportaltask" title='. esc_html(__('approve', 'wp-job-portal')).'>
                                '. esc_html(__('Job Approve', 'wp-job-portal')).'
                            </a>';
                        }
                        if (isset($arr['feature']) && in_array('featuredjob', wpjobportal::$_active_addons)) {
                           
                            $html.='<a class="wpjobportal-jobs-act-btn" href="admin.php?page=wpjobportal_job&task=approveQueueFeaturedJob&id='.$job->id.'&action=wpjobportaltask" title='. esc_html(__('approve', 'wp-job-portal')).'>
                    ...
                    if ($total == 1) {
                        if (isset($arr['self'])) {
                            $html.='<a class="wpjobportal-jobs-act-btn" href="admin.php?page=wpjobportal_job&task=rejectQueueJob&id='. $job->id.'&action=wpjobportaltask" title='.  esc_html(__('reject', 'wp-job-portal')).'>
                                '.  esc_html(__('Job Reject', 'wp-job-portal')).'
                            </a>';
                        }
                        if (isset($arr['feature']) && in_array('featuredjob', wpjobportal::$_active_addons)) {
                            $html.='<a class="wpjobportal-jobs-act-btn" href="admin.php?page=wpjobportal_job&task=rejectQueueFeaturedJob&id='. $job->id.'&action=wpjobportaltask" title='.  esc_html(__('reject', 'wp-job-portal')).'>
                    ...
                    $html.='
                    <a class="wpjobportal-jobs-act-btn" href='. esc_url(admin_url('admin.php?page=wpjobportal_job&wpjobportallt=formjob&wpjobportalid='.$job->id)).' title='.  esc_html(__('edit', 'wp-job-portal')).'>
                        '. esc_html(__('Edit', 'wp-job-portal')).'
                    </a>
                    <a class="wpjobportal-jobs-act-btn" href='. wp_nonce_url(admin_url('admin.php?page=wpjobportal_job&task=remove&wpjobportal-cb[]='.$job->id),'delete-job').'&action=wpjobportaltask&callfrom=2 onclick="return confirm(\''. esc_html(__('Are you sure to delete','wp-job-portal')) . ' ?'.'\');" title='.esc_html(__('delete', 'wp-job-portal')).'>
