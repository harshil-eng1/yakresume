...
class oauth_client_class {
    ...
    Function GetRequestState(&$state) {
        ...
        $state = (IsSet($_GET[$check]) ? filter_var($_GET[$check], FILTER_SANITIZE_STRING) : null);
        return(true);
    }
    ...
    Function GetRequestError(&$error) {
        $error = (IsSet($_GET['error']) ? filter_var($_GET['error'], FILTER_SANITIZE_STRING) : null);
        return(true);
    }

    Function GetRequestDenied(&$denied) {
        $denied = (IsSet($_GET['denied']) ? filter_var($_GET['denied'], FILTER_SANITIZE_STRING) : null);
        return(true);
    }

    Function GetRequestToken(&$token, &$verifier) {
        $token = (IsSet($_GET['oauth_token']) ? filter_var($_GET['oauth_token'], FILTER_SANITIZE_STRING) : null);
        $verifier = (IsSet($_GET['oauth_verifier']) ? filter_var($_GET['oauth_verifier'], FILTER_SANITIZE_STRING) : null);
        return(true);
    }
                    ...
                    if (strlen($this->authorization_error)) {
                        ...
                        <p>It was not possible to authorize the application.<?php
                            if ($this->debug) {
                                ...
                                echo esc_html(HtmlSpecialChars($this->authorization_error));
                            }
                            ?></p>
                        <?php
                        ...
                        <p>It was not possible to use the application access token.
                            ...
                            if ($this->debug) {
                                ?>
                                <br>Error: <?php
                                echo esc_html(HtmlSpecialChars($this->access_token_error));
                            }
                            ?></p>
                        <?php
                        ...
                        <p>The application authorization was obtained successfully.
                            ...
                            if ($this->debug) {
                                ?>
                                <br>Access token: <?php
                                echo esc_html(HtmlSpecialChars($this->access_token));
                                if (IsSet($this->access_token_secret)) {
                                    ?>
                                    <br>Access token secret: <?php
                                    echo esc_html(HtmlSpecialChars($this->access_token_secret));
                                }
                            }
                            ?></p>
                        <?php
