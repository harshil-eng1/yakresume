    ...
    $inline_js_script = "
        ...
        if(in_array('credits', wpjobportal::$_active_addons)){ 
            ...
            function getPackagePopup(jobid,themecall) {
                ...
                var ajaxurl = \"". admin_url('admin-ajax.php') ."\";
                jQuery.post(ajaxurl, {action: 'wpjobportal_ajax', wpjobportalme: 'featuredjob', task: 'getPackagePopupForFeaturedJob', wpjobportalid: jobid, '_wpnonce':'". esc_attr(wp_create_nonce("get-package-popup-for-featured-job"))."'}, function (data) {
                    if (data) {
            ...
            function getPackageCopyPopup(jobid,themecall){
                var themecall = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
                var ajaxurl = \"". admin_url('admin-ajax.php') ."\";
                jQuery.post(ajaxurl, {action: 'wpjobportal_ajax', wpjobportalme: 'copyjob', task: 'getPackagePopupForCopyJob', wpjobportalid: jobid, '_wpnonce':'". esc_attr(wp_create_nonce("get-package-popup-for-copy-job"))."'}, function (data) {
                    if (data) {
                        if(jQuery('#package-popup').length)
                        jQuery('#package-popup').remove();
                        jQuery('body').append(data);
