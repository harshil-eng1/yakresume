    ...
    $inline_js_script = "
        ...
        jQuery(document).ready(function () {
            jQuery('select#companyid').on('change', function() {
                companycall = companycall + 1;
                var companyid = this.value;
                jQuery.post(\"". admin_url('admin-ajax.php')."\",{action:'wpjobportal_ajax',wpjobportalme:'user',task:'getUserIdByCompanyid',companyid:companyid, '_wpnonce':'". esc_attr(wp_create_nonce("get-user-id-by-company-id"))."'},function(data){
                    if(data){
            ...
            <form id="job_form" class="wpjobportal-form" method="post" action="<?php echo admin_url("admin.php?page=wpjobportal_job&task=savejob"); ?>">
                <?php if (isset(wpjobportal::$_data[0]->msg) AND wpjobportal::$_data[0]->msg != '') { ?>
                    <span class="formMsg">
                        <font color="red">
                            <strong>
                                <?php echo wpjobportal::wpjobportal_getVariableValue(wpjobportal::$_data[0]->msg); ?>
                            </strong>
                        </font>
                    </span>
                <?php } ?>
                ...
                <div class="wpjobportal-form-button">
                    <a id="form-cancel-button" class="wpjobportal-form-cancel-btn" href="<?php echo admin_url('admin.php?page=wpjobportal_job'); ?>" title="<?php echo esc_html(__('cancel', 'wp-job-portal')); ?>">
